...
if hardest_sampling:
    ...
    temp_str = 'rand_'
# Build the name
MODEL_NAME = temp_str+str(data_args['nn_sampling'])+'-'+str(data_args['output_type'])+'-'+str(data_args['image_size_mt'])+'-'+str(data_args['image_size_px'])+'-'+str(data_args['orientation'])+'-'+str(data_args['batch_size'])+'_'
...
def main():
    ...
    if general_args['generate_latent_vis'] or general_args['load_model']:
        ...
        x_image = np.zeros([len(dataloader.dataset),1,data_args['image_size_px'],data_args['image_size_px']])
        x_trajectory = np.zeros([len(dataloader.dataset),30,3])
